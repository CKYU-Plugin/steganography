...
namespace F5
{
    ...
    public class JpegExtract : IDisposable
    {
        ...
        public JpegExtract(Stream output, string password, bool dispose = false)
            : this(output, Encoding.ASCII.GetBytes(password), dispose)
        {
        }

        public JpegExtract(Stream output, byte[] password, bool dispose)
        {
            this.output = output;
        ...
        public bool Extract(Stream input)
        {
            int[] coeff;
            int i, n, k, hash, code;
